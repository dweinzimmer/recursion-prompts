...
var sum = function(array) {
  ...
  // recursive case: pass in slice of remainder of array after [0]
...
var fibonacci = function(n) {
  ...
  var prevTwo = fibonacci(n - 1).slice(-2);

  return fibonacci(n - 1).concat(prevTwo[0] + prevTwo[1]);
};
...
var capitalizeFirst = function(array) {
  var processedValue = array[0][0].toUpperCase() + array[0].slice(1) || '';
  if (array.length === 1) {
    return [processedValue];
  }

  return [processedValue].concat(capitalizeFirst(array.slice(1)));
};
...
var flatten = function(array) {
  ...
  } else if (array[0][0] !== undefined) {
    // if first item is an array and it's not empty, flatten it
    return flatten(array[0]).concat(flatten(array.slice(1)))
  } else {
    // handle empty array values by skipping them
...
var alternateSign = function(array) {
  // base case: if no pairs left, return nothing
  if (array.length === 0) {
    return array;
  }
